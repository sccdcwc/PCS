...
namespace PCS.BLL
{
    class SQLiteBLL
    {
        ...
        #region Sqlite实际操作
        public void DZInsert(string sYH_GUID, string sPath)
        {
            string sql = string.Format("insert into YH_PZB ('YH_GUID','PZMC','PZLX','PZMRZ','PZNRZ','PZGXSJ') values ('{0}','文件夹本地地址','同步配置','','{1}',(select datetime('{2}')))",sYH_GUID,sPath,ChangeTime(DateTime.Now));
            ExecuteSql(sql);        
        }
        #endregion
    }
