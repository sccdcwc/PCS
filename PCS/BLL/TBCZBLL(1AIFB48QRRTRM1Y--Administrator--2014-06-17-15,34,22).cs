    ...
    public class TBCZBLL
    {
        ...
        public void AddJCML(UserModel user)
        {
            ...
            for (int i = 0; i < dt1.Rows.Count; i++)
            {
                ...
                {
                    string sSql3 = string.Format("select * from YH_BDML where MLBM='{0}'", dt2.Rows[l]["MLBM"]);
                    if (!sqlite.IsHaveInSqlite(sSql3))
                    {
                        string sSql4 = string.Format("insert into YH_BDML ('FJMLBM','SFYZML','MLBM','MLBTMC','QYZT','PXH') values ('{0}','{1}','{2}','{3}','{4}','{5}')", dt2.Rows[l]["FJMLBM"].ToString(), dt2.Rows[l]["SFYZML"].ToString(), dt2.Rows[l]["MLBM"].ToString(), dt2.Rows[l]["MLBTMC"].ToString(), dt2.Rows[l]["QYZT"].ToString(), dt2.Rows[l]["PXH"].ToString(), dt2.Rows[i]["MLBTMCQC"]);
                        sqlite.ExecuteSql(sSql4);
                    }
                    string sSql5 = string.Format("select * from YH_BDML where FJMLBM='{0}'", dt2.Rows[l]["MLBM"]);
                    if (!sqlite.IsHaveInSqlite(sSql5))
                    {
                        string sSql6 = string.Format("select * from jc_ml where fjmlbm='{0}'", dt2.Rows[l]["MLBM"]);
                        stream = client.GetTable(sSql6);
                        DataTable dt3 = cs.DeSerializeBinary(stream) as DataTable;
                        for (int m = 0; m < dt3.Rows.Count; m++)
                        {
                            string sSql7 = string.Format("insert into YH_BDML ('FJMLBM','SFYZML','MLBM','MLBTMC','QYZT','PXH') values ('{0}','{1}','{2}','{3}','{4}','{5}')", dt3.Rows[m]["FJMLBM"].ToString(), dt3.Rows[m]["SFYZML"].ToString(), dt3.Rows[m]["MLBM"].ToString(), dt3.Rows[m]["MLBTMC"].ToString(), dt3.Rows[m]["QYZT"].ToString(), dt3.Rows[m]["PXH"].ToString(), dt3.Rows[m]["MLBTMCQC"]);
        ...
        public void GXWJJ(UserModel user)
        {
            ...
            if (dt.Rows.Count>0)
            {
                ...
                string sSql2 = string.Format("select * from yh_bdml wehre yh_guid='{0}' and GXSJ>datetime('{1}')", user.YH_GUID, time);
                DataTable dt2 = sqlite.GetTable(sSql2);
                for (int i = 0; i < dt2.Rows.Count; i++)
                {
                    //获取该条消息在服务端的信息
                    string sSql3 = string.Format("select * from yh_ml where yh_guid={0} and mlbm='{1}'", user.YH_GUID, dt2.Rows[i]["MLBM"]);
                    ...
                    if (dt3.Rows.Count > 0)
                    {
                        ...
                        if (WLtime > BDtime)
                        {
                            string sUpdateSql1 = string.Format("update YH_BDML set FJMLBM='{0}',MLBTMC='{1}',SFYZML='{2}',QYZT='{3}',GXSJ=(select datetime('{4}')),BZ='{5}' where YH_GUID='{6}' and MLBM='{7}'", dt3.Rows[0]["FJMLBM"].ToString(), dt3.Rows[0]["MLBTMC"].ToString(), dt3.Rows[0]["SFYZML"].ToString(), dt3.Rows[0]["QYZT"].ToString(), sqlite.ChangeTime(Convert.ToDateTime(dt3.Rows[0]["GXSJ"])), dt3.Rows[0]["BZ"].ToString(), user.YH_GUID, dt3.Rows[0]["MLBM"]);
                            sqlite.ExecuteSql(sUpdateSql1);
                        }
                        else
                        {
                            //本地文件目录更新时间比服务器新
                            string sUpdateSql2 = string.Format("update YH_ML set FJMLBM='{0}',MLBTMC='{1}',SFYZML='{2}',QYZT='{3}',GXSJ=to_date('{4}','YYYY-MM-DD HH24:MI:SS'),BZ='{5}' where YH_GUID='{6}' and MLBM='{7}'", dt2.Rows[i]["FJMLBM"].ToString(), dt2.Rows[i]["MLBTMC"].ToString(), dt2.Rows[i]["SFYZML"].ToString(), dt2.Rows[i]["QYZT"].ToString(), sqlite.ChangeTime(Convert.ToDateTime(dt2.Rows[i]["GXSJ"])), dt2.Rows[i]["BZ"].ToString(), user.YH_GUID, dt2.Rows[i]["MLBM"]); ;
                            client.ExcuteSql(sUpdateSql2);
                        }
                    }
                    else
                ...
                {
                    //获取该条消息在本地端的信息
                    string sSql3 = string.Format("select * from yh_bdml where yh_guid={0} and mlbm='{1}'", user.YH_GUID, dt2.Rows[i]["MLBM"]);
            ...
            else
            {
                ...
                if (dt1.Rows.Count > 0)
                {
                    for (int i = 0; i < dt1.Rows.Count;i++)
                    {
                        string sInsertSql = string.Format("insert into YH_ML (YH_GUID,FJMLBM,SFYZML,MLBM,MLBTMC,CJSJ,GXSJ,QYZT,PXH,BZ) values ('{0}','{1}','{2}','{3}','{4}',to_date('{5}','YYYY-MM-DD HH24:MI:SS'),to_date('{6}','YYYY-MM-DD HH24:MI:SS'),'{7}','{8}','{9}')", dt1.Rows[i]["YH_GUID"].ToString(), dt1.Rows[i]["FJMLID"].ToString(), dt1.Rows[i]["SFYZML"].ToString(), dt1.Rows[i]["MLBM"].ToString(), dt1.Rows[i]["MLBTMC"].ToString(), sqlite.ChangeTime(Convert.ToDateTime(dt1.Rows[i]["CJSJ"])), sqlite.ChangeTime(Convert.ToDateTime(dt1.Rows[i]["GXSJ"])), dt1.Rows[i]["QYZT"].ToString(), dt1.Rows[i]["PXH"].ToString(), dt1.Rows[i]["BZ"].ToString());
                        sqlite.ExecuteSql(sInsertSql);                  
                    }
                }
